...
export function detectHistoricalRuntime(cwd){const explicit=fileNodeMajor(cwd),workflow=workflowNodeMajors(cwd),pkg=packageNodeMajor(cwd),current=Number(process.versions.node.split('.')[0]);let selected=null,source=null;if(explicit){selected=explicit.major;source=explicit.source;}else if(workflow.length){selected=workflow[0];source='historical-ci';}else if(pkg){selected=pkg.major;source=pkg.source;}const selectedNodeMajor=selected||current,pm=detectHistoricalPackageManager(cwd,selectedNodeMajor);return{currentNodeMajor:current,selectedNodeMajor,source:source||'current-runtime',workflowNodeMajors:workflow,engineRange:pkg?.range||null,usesHistoricalNode:Boolean(selected&&selected!==current),packageManager:pm.name,packageManagerVersion:pm.version,packageManagerSource:pm.source};}
...
export function runtimeSummary(runtime){const tested=runtime.workflowNodeMajors.length?` · historical CI [${runtime.workflowNodeMajors.join(', ')}]`:'';const engines=runtime.engineRange?` · engines ${runtime.engineRange}`:'';const pm=runtime.packageManager?` · ${runtime.packageManager}@${runtime.packageManagerVersion} (${runtime.packageManagerSource})`:'';return`Node ${runtime.selectedNodeMajor} (${runtime.source})${tested}${engines}${pm}`;}
