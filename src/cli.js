...
function evolve(){
  ...
  console.log(`\nAgent: ${agent.agent}${agent.model?` · ${agent.model}`:''}`);if(agent.agent==='openrouter')console.log(`OpenRouter turn budget: ${agent.maxTurns}`);console.log(`Qualified task split: ${training.length} training · ${heldout.length} held-out`);console.log(`Evolution tournament: ${mutations.length} mutation candidates · ${trials} paired stochastic trials`);if(options.installDependencies)console.log('Dependency preparation: historical lockfiles, commit-date resolution, and legacy two-phase resolution enabled.');console.log('Integrity rule: first trial explores candidates; the winner must improve a majority of paired training trials with zero regressions, then remain non-regressing in every paired held-out trial. Exact task IDs stay locked throughout.');
