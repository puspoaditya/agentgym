...
export function discoverQualifiedTasksSmart(repo,{limit=10,scanLimit=Math.max(limit*10,50),installDependencies=false,historicalRuntime=true}={}){
  const r=shell('git',['log',`--max-count=${scanLimit}`,'--format=%H%x09%s'],{cwd:repo,allowFailure:true}),counts={qualified:0,'dependency-failure':0,'no-verification':0,'no-regression':0,'unstable-regression':0,'patch-apply-failure':0,'post-patch-dependency-failure':0,'ground-truth-verification-failure':0};
  if(r.status!==0)return{tasks:[],scanned:0,rejected:0,counts,prioritized:0,diagnostics:[],runtimeCounts:{},dependencyFailureCounts:{},verificationFailureCounts:{},resolutionCounts:{}};
  const commits=r.stdout.split('\n').filter(Boolean).map((line,index)=>{const[hash,...parts]=line.split('\t');return{hash,subject:parts.join('\t'),index};}),ordered=prioritizeCommits(commits),tasks=[],diagnostics=[],runtimeCounts={},dependencyFailureCounts={},verificationFailureCounts={},resolutionCounts={};let scanned=0,prioritized=0;
  for(const commit of ordered){
    ...
    const qualification=qualifyTaskDetailed(repo,task,{installDependencies,stabilityRuns:2,groundTruthRuns:2,historicalRuntime}),reason=qualificationReason(qualification);counts[reason]=(counts[reason]||0)+1;
  ...
  return{tasks,scanned,rejected:scanned-tasks.length,counts,prioritized,diagnostics,runtimeCounts,dependencyFailureCounts,verificationFailureCounts,resolutionCounts};
}
